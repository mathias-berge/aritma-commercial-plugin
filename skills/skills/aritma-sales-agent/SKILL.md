...
Når brukeren spør om forecast, anslått closing, win rate, pipeline-sannsynlighet eller andre deal-statistikker:
...
- Bruk alltid **faktisk historisk konverteringsrate** fra HubSpot som grunnlag, ikke pipeline-weighting eller manuelle probability-felt (`hs_deal_stage_probability`, `hs_forecast_probability`). Beregn som: antall deals med `hs_is_closed_won = true` delt på antall opprettede deals i samme periode (standard: siste 12 måneder), for relevant pipeline.
...
- Vær eksplisitt på hvilke antakelser som brukes: skill mellom faktisk historisk conversion rate, estimater og ren pipeline-verdi. Presenter aldri forecast som en garanti.
- Eksempelformulering: "Basert på faktisk conversion rate siste 12 måneder i HubSpot, hvor X % av opprettede deals i Sales Pipeline har blitt Closed Won, er estimert closing på nåværende pipeline omtrent NOK Y."
